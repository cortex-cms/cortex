angular.module('cortex.controllers.users.bulk', [
  'ui.router.state',
  ...
  .controller('UsersBulkCtrl', function ($scope, $timeout, $upload, $state, $q, flash, cortex, settings) {
    $scope.data = $scope.data || {};
    $scope.data.usersBulk = new cortex.users.bulkJob();
    $scope.data.upload = {
      progress: 0
    };
    ...
    function saveBulkJob() {
      ...
      if (!$scope.data.usersBulk.metadata) {
        ...
        var httpConfig = {
          ...
          method: 'POST',
          fields: {bulkJob: $scope.data.usersBulk},

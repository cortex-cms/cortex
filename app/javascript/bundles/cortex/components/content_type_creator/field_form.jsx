...
import {CLOSE_FIELD_EDITOR, UPDATE_YAML_FIELD, FIELD_NAME_ERROR, ADD_FIELD, UPDATE_FIELD_FORM, UPDATE_FIELD, EXPAND_DEFAULTS} from '../../constants/content_type_creator'
...
class FieldForm extends React.PureComponent {
            ...
            <FormControlLabel
                ...
                control={
                  <Checkbox
                    ...
                    onChange={this.fieldDefaultClicked(dataFieldKey, {[propName]: defaultValues[propName] }, propName)}
                    value={propName}
                  />
                }
                label={propName}
  ...
  fieldDefaultClicked = (dataFieldKey, defaultProp, propName) => (event) => {
    ...
    if (dataField[propName] === undefined) {
      ...
      this.props.dispatch({type: UPDATE_YAML_FIELD, payload: { [dataFieldKey + 'Yaml']: dataFieldYaml  + YAML.stringify(defaultProp, 2)}, updateFieldView: { [dataFieldKey]: dataField }  })
    }
    else {
      this.props.dispatch({type: UPDATE_YAML_FIELD, payload: { [dataFieldKey + 'Yaml']: YAML.stringify(this.removeField(parseYAMLValue(dataFieldYaml), propName), 2) }, updateFieldView: { [dataFieldKey]: this.removeField(dataField, propName) }})
    }
  }
  nameUnique = (name, field_edit) => this.props.fields.reduce((bool, field, i) => {
    if (bool === false || i === field_edit) return bool
    return field.name !== name
  }, true)
  addFieldToFieldType = () => {
    const { name = '', field_edit } = this.props;
    if(name.length < 2){
  ...
  updateYamlField = (fieldDataKey) => (newValue) => this.props.dispatch({type: UPDATE_YAML_FIELD, payload: { [fieldDataKey + 'Yaml']: newValue }, updateFieldView: {} })
    ...
    <div className='mdl-cell mdl-cell--12-col'>
      ...
      <FormControl fullWidth className=''>
        <AceEditor
          ...
          value={YAML.stringify(this.treeFieldValues(), 2)}
          onChange={this.updateYamlField()}
          name='tree_values'
            ...
            <div className='mdl-cell mdl-cell--6-col'>
              ...
              <FormControl fullWidth className=''>
                <AceEditor
                  ...
                  highlightActiveLine={true}
                  width='100%'
                  ref={input => this.validations = input}
                  value={validationsYaml}
                  onChange={this.updateYamlField('validations')}
            ...
            <div className='mdl-cell mdl-cell--6-col'>
              ...
              <FormControl fullWidth className=''>
                <AceEditor
                  ...
                  highlightActiveLine={true}
                  value={metadataYaml}
                  onChange={this.updateYamlField('metadata')}
                  name='metadata'
                  setOptions={this.ReactAceOptions}
                  editorProps={this.AceEditorProps}/>
              </FormControl>

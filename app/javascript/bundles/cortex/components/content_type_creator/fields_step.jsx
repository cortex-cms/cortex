...
import { ADD_FIELD, OPEN_FIELD_FORM, EDIT_FIELD, DELETE_FIELD, CLOSE_ADD_FIELD, OPEN_FIELD_EDITOR, UPDATE_FIELD } from '../../constants/content_type_creator'
...
class FieldsStep extends React.PureComponent {
  ...
  changeFieldType = (e) => this.props.dispatch({ type: UPDATE_FIELD, payload: { field_type: e.target.value } })
  ...
  renderFields = () => this.props.data.map((field, index) => {
   ...
   return(
     <TableRow key={index}>
       ...
       <TableCell><b>{field_type}</b></TableCell>
  ...
  render() {
    ...
    return (
      ...
      <Paper>
        <button className={ field_builder.open ? 'hidden' : 'mdl-button mdl-js-button mdl-button--primary content-type-new-field-button' } onClick={ this.addField }>
          New Field
        </button>

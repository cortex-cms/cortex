module Cortex
  class ContentItemService < ApplicationService
    include Cortex::WidgetParsersHelper
    ...
    def update
      transact_and_refresh do
        @content_item = Cortex::ContentItem.find(id)
        content_item_params['field_items_attributes'].to_hash.each do |key, field_item_attributes|
          @content_item.field_items << Cortex::UpdateFieldItemTransaction.new.call(field_item_attributes).value
        end

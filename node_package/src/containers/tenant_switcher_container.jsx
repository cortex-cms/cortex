...
import TenantLookup from '../helpers/tenant_lookup'
...
class TenantSwitcherContainer extends React.PureComponent {
  ...
  updateTenant = (tenant) => {
    ...
    this.railsAPI.post('/admin_update/tenant_change', {
      ...
      self.props.dispatch({
        type: TENANT_UPDATED,
        payload: Object.assign({}, currentUser, {active_tenant: response.data})
      })
      self.layoutWrapper.classList.remove('sidebar--tenant-display')
  ...
  toggleTenantSwitcher = () => {
    ...
    this.layoutWrapper.className = !this.props.data.tenantListActive ? 'sidebar--tenant-display' : '';
  }
  toggleSidebar = () => {
      ...
      <footer id="tenant_switch">
        ...
        <nav className='demo-navigation mdl-navigation'>
          ...
          <div onClick={this.toggleTenantSwitcher} className='mdl-navigation__link nav__item'>
            ...
            <i className='material-icons tenant-toggle'
               role="presentation">{tenantListActive === true ? 'expand_less' : 'expand_more'}</i>
          </div>
        </nav>

...
describe Tree, :type => :model do
  describe '::recursive_search' do
    ...
    it 'should return an Array' do
      expect(Tree.recursive_search(tree_array)).to be_a_kind_of(Array)
    end

    it 'should include all the ids from the given Array' do
      expect(Tree.recursive_search(tree_array)).to include(1)
      expect(Tree.recursive_search(tree_array)).to include(2)
      expect(Tree.recursive_search(tree_array)).to include(3)
      expect(Tree.recursive_search(tree_array)).to include(4)
      expect(Tree.recursive_search(tree_array)).to include(5)
    end
  end
end
